...
while True:
    ...
    if previous_eye_state == "eyes open" and eye_state == "eyes closed":
        ...
        if eye_closed_start_time:  # If eyes were closed before
            ...
            if blink_duration >= 1.5:  # If eyes were closed for ≥ 1.5 seconds (long blink)
                long_blink_count += 1
    ...
    cv2.putText(frame, f"Long Blinks/min: {long_blink_rate}", (50, 110),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2, cv2.LINE_AA)

    # Drowsiness & Alert Conditions
    status = "AWAKE"  # Default state
    cnt.out('A')
    if blink_rate >= 30 and yawn_rate >= 3:
        status = "DROWSY"
        cnt.out('D')
    elif long_blink_rate >= 2 and long_blink_rate < 5:
        status = "DROWSY"
        cnt.out('D')
    elif long_blink_rate >= 4:
        status = "ALERT"
        cnt.out('Al')
    ...
    cv2.putText(frame, status, (50, 150),
                ...
                (0, 0, 255) if status in ["DROWSY", "ALERT"] else (0, 255, 0),
                3, cv2.LINE_AA)
